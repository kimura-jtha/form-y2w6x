...
export function usePrizeClaimForm() {
  ...
  const form = useForm<PrizeClaimFormValues>({
    ...
    validate: {
      ...
      bankCode: (value) => (!value ? t('prizeClaim.validation.required') : null),
      branchCode: (value) => (!value ? t('prizeClaim.validation.required') : null),
      accountType: (value) => (!value ? t('prizeClaim.validation.required') : null),
      accountNumber: (value) => {
        if (!value.trim()) return t('prizeClaim.validation.required');
      ...
      accountHolderName: (value) => (!value.trim() ? t('prizeClaim.validation.required') : null),
      privacyAgreed: (value) =>
